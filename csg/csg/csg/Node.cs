...
namespace csg
{
    public class Node
    {
        ...
        public Node front;
        public Node back;
        public List<CsgPolygon> polygons = new List<CsgPolygon>(); // TODO: optimization: this could probably be an array

        public Node(List<CsgPolygon> polys)
        {
            List<CsgPolygon> tpoly = new List<CsgPolygon>();
            foreach (CsgPolygon p in polys)
                tpoly.Add(p.clone());
            build(tpoly);
        }
        ...
        /// <returns></returns>
        public List<CsgPolygon> clipPolygons(List<CsgPolygon> polys)
        {
            if (this.plane == null) return new List<CsgPolygon>(polys);
        ...
        {
            List<CsgPolygon> polys = new List<CsgPolygon>(this.polygons);
        ...
        public void build(List<CsgPolygon> polys, int stack = 6000)
        {
            if (stack < 0)
            {
                return;
            ...
            List<CsgPolygon> tfront = new List<CsgPolygon>();
            List<CsgPolygon> tback = new List<CsgPolygon>();
            for (int i = 0; i < polys.Count; i++)
            {
